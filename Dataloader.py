...
def prepare_event_data_dict(list_of_events):
    ...
    for event_idx in range(len(list_of_events)):
        ...
        event_data_dict[event_id] = {
            ...
            "time_abs": event_time_abs,
            "time_rel": event_time_rel,
            "type": event_type,
        }
    return list_of_event_ids, event_data_dict


def check_if_any_event_in_range(
    list_of_event_times_datetimes,
    list_of_event_ids,
    event_data_dict,
    start_time,
    end_time,
...
def prepare_data_loader(
    ...
    if labels_file_path is not None:
        ...
        list_of_event_times_datetimes = [
            datetime.strptime(
                event_data_dict[event_id]["time_abs"], "%Y-%m-%dT%H:%M:%S.%f"
            )
    ...
    for file_idx in range(len(list_of_files)):
        ...
        if days_difference == 1:
            ...
            while not end_of_file:
                ...
                if sampling_rate != 6.625:
                    tmp_data = signal.resample(
                        tmp_data, int(len(tmp_data) * 6.625 / sampling_rate)
                    )

                tmp_data_undersample = signal.decimate(
                    tmp_data, decimation_factor, axis=0, zero_phase=True
                )
                _, _, sxx = signal.spectrogram(
                    tmp_data_undersample,
                    6.625 / decimation_factor,
                    nfft=spect_nfft,
                    nperseg=spect_nperseg,
                )
                ...
                if labels_file_path is not None:
                    if "evNONE" in list_of_files[file_idx]:
                        ...
                        label, event_type = check_if_any_event_in_range(
                            list_of_event_times_datetimes,
                            list_of_event_ids,
                            event_data_dict,
                            start_time,
            ...
            spectrograms_np_arr = (
                spectrograms_np_arr - np.mean(spectrograms_np_arr)
            ) / np.std(spectrograms_np_arr)
    ...
    if labels_file_path is not None:
        ...
        train_dataset = utils.TensorDataset(
            torch.from_numpy(all_spectrograms).float(),
            torch.from_numpy(all_labels).bool(),
            torch.from_numpy(all_event_types).int(),
        )
        ...
        train_loader = utils.DataLoader(
            train_dataset, batch_size=batch_size, drop_last=False, sampler=sampler
        )
